...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out = X
        for i in range(self.num_layers):
          out,c_aff = affine_forward(out,self.params[f'W{i+1}'],self.params[f'b{i+1}'])
          c_bn = ()
          c_relu = ()
          c_drop = ()
          if i!=self.num_layers-1:
            if self.normalization == "batchnorm":
              out,c_bn = batchnorm_forward(
                out,
            ...
            if self.use_dropout and self.dropout_param["p"] != 1:
              out,c_drop = dropout_forward(out,self.dropout_param)
          cache.append((c_aff,c_relu,c_bn,c_drop))
        scores = out
        ...
        for i in range(self.num_layers,0,-1):
          c_aff,c_relu,c_bn,c_drop = cache[i-1]
          if i==self.num_layers:
            dx,grads[f'W{i}'],grads[f'b{i}'] = affine_backward(dx,c_aff)
            grads[f'W{i}'] += self.reg*self.params[f'W{i}']
            continue
          else:
            if self.use_dropout and self.dropout_param["p"]!=1:
              dx = dropout_backward(dx,c_drop)
            dx = relu_backward(dx,c_relu)
            if self.normalization == "batchnorm":
